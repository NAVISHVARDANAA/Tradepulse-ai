...
1. Open **Actions → Build production web release**, choose `main`, enter
   `BUILD_PHASE_6C` and record the green live-trading readiness web artifact.
2. Open **Actions → Deploy controlled beta web**, choose `main`, enter
   `DEPLOY_PHASE_6C` and approve the protected production environment.
...
4. After a release or operational incident, open **Actions → Verify web
   production**, choose `main`, enter `VERIFY_WEB_PHASE_6C` and retain the green
...
Deploy and verify migration 039 with the Phase 6C data gates before deploying the
web workspace. The readiness ledger is read-only in the browser and cannot
activate a live order route, cohort or external invitation.
