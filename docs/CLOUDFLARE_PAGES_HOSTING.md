...
1. Open **Actions → Deploy Supabase production**, choose `main`, enter
   `DEPLOY_DATA_PHASE_8J` and wait for it to pass.
2. Run **Verify Supabase production** on `main` with
   `VERIFY_DATA_PHASE_8J` and retain the read-only evidence.
3. Open **Actions → Build production web release**, choose `main`, enter
   `BUILD_PHASE_8J` and record the green global-dependency-transmission web artifact.
4. Open **Actions → Deploy controlled beta web**, choose `main`, enter
   `DEPLOY_PHASE_8J` and approve the protected production environment.
...
6. After a release or operational incident, open **Actions → Verify web
   production**, choose `main`, enter `VERIFY_WEB_PHASE_8J` and retain the green
...
Deploy and verify migration 054 with the Phase 8J data gates before the web
workspace. Every country-domain relationship remains an explicit gap until
licensed, directed and reconciled evidence passes human review. The product
cannot infer dependencies, score impacts, promote scenarios, publish or train
on external claims, route an order, move money, hold assets or settle a trade.
