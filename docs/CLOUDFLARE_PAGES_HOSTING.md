...
1. Open **Actions → Deploy Supabase production**, choose `main`, enter
   `DEPLOY_DATA_PHASE_8M` and wait for it to pass.
2. Run **Verify Supabase production** on `main` with
   `VERIFY_DATA_PHASE_8M` and retain the read-only evidence.
3. Open **Actions → Build production web release**, choose `main`, enter
   `BUILD_PHASE_8M` and record the green provider contract-test web artifact.
4. Open **Actions → Deploy controlled beta web**, choose `main`, enter
   `DEPLOY_PHASE_8M` and approve the protected production environment.
...
6. After a release or operational incident, open **Actions → Verify web
   production**, choose `main`, enter `VERIFY_WEB_PHASE_8M` and retain the green
...
Deploy and verify migration 057 with the Phase 8M data gates before the web
workspace. Every provider family remains unselected, every isolated-intake
profile remains unprovisioned, and all twenty-four synthetic fixtures remain
unexecuted until legal, rights, privacy, security, schema, isolation and
failure-drill evidence passes human review. The product cannot test an endpoint,
