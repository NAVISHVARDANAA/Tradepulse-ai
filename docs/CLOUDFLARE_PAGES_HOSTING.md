...
1. Open **Actions → Deploy Supabase production**, choose `main`, enter
   `DEPLOY_DATA_PHASE_8N` and wait for it to pass.
2. Run **Verify Supabase production** on `main` with
   `VERIFY_DATA_PHASE_8N` and retain the read-only evidence.
3. Open **Actions → Build production web release**, choose `main`, enter
   `BUILD_PHASE_8N` and record the green provider evidence-review web artifact.
4. Open **Actions → Deploy controlled beta web**, choose `main`, enter
   `DEPLOY_PHASE_8N` and approve the protected production environment.
...
6. After a release or operational incident, open **Actions → Verify web
   production**, choose `main`, enter `VERIFY_WEB_PHASE_8N` and retain the green
...
Deploy and verify migration 058 with the Phase 8N data gates before the web
workspace. Every provider review packet remains unopened, all ninety-six
evidence cells remain missing and every Phase 8M synthetic fixture remains
unexecuted until legal, rights, privacy, security, schema, isolation and
failure-drill evidence passes human review. The product cannot select a provider,
open a review packet, submit evidence, test an endpoint,
