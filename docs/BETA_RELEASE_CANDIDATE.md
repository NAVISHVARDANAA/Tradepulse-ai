...
- Brokerage-readiness previews, payment-compliance maps, beneficiary-protection rehearsals and transparent corridor comparisons that cannot execute.
...
These are operational, legal and business launch decisions. Passing Phase 7C
...
After the Phase 7C PR is merged and all `main` checks pass:

1. Deploy and verify migration 042 with `DEPLOY_DATA_PHASE_7C` and `VERIFY_DATA_PHASE_7C`.
2. Open **Actions → Build production web release**.
3. Select `main`, enter `BUILD_PHASE_7C` and run the workflow.
...
6. Run **Verify web production** with `VERIFY_WEB_PHASE_7C` after deployment
...
Phase 7C adds migration 042 and a sanitized compliance-orchestration map; deploy
and verify the data boundary before the web release. Real identity and document
collection, provider screening, monitoring, travel-rule transmission, case
writes, clearance, overrides, transfers and money movement remain database-locked off.
