...
for (const [name, workflow, confirmation] of [
  ['build', buildWorkflow, 'BUILD_PHASE_5G'],
  ['deploy', deployWorkflow, 'DEPLOY_PHASE_5G'],
  ['verify', verifyWorkflow, 'VERIFY_WEB_PHASE_5G'],
]) {
  assert(workflow.includes(confirmation), `Phase 5F ${name} confirmation is missing`)
  assert(workflow.includes('check:beta-operations'), `Phase 5G ${name} omits the operations contract`)
}
