...
assert(manifest.phase==='8G'&&manifest.status==='controlled_live_rollout_candidate','Manifest is not Phase 8G')
...
for(const [workflow,token] of [[deployData,'DEPLOY_DATA_PHASE_8G'],[verifyData,'VERIFY_DATA_PHASE_8G'],[buildWeb,'BUILD_PHASE_8G'],[deployWeb,'DEPLOY_PHASE_8G'],[verifyWeb,'VERIFY_WEB_PHASE_8G']]) assert(workflow.includes(token),`Workflow omits ${token}`)
