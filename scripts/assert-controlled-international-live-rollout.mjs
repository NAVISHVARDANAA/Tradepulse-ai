...
assert(manifest.phase==='8M'&&manifest.status==='global_provider_contract_test_lab_candidate','Manifest is not Phase 8G')
...
for(const [workflow,token] of [[deployData,'DEPLOY_DATA_PHASE_8M'],[verifyData,'VERIFY_DATA_PHASE_8M'],[buildWeb,'BUILD_PHASE_8M'],[deployWeb,'DEPLOY_PHASE_8M'],[verifyWeb,'VERIFY_WEB_PHASE_8M']]) assert(workflow.includes(token),`Workflow omits ${token}`)
