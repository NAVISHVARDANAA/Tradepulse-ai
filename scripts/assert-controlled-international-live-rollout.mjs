...
assert(manifest.phase==='8K'&&manifest.status==='global_observation_provenance_quarantine_candidate','Manifest is not Phase 8G')
...
for(const [workflow,token] of [[deployData,'DEPLOY_DATA_PHASE_8K'],[verifyData,'VERIFY_DATA_PHASE_8K'],[buildWeb,'BUILD_PHASE_8K'],[deployWeb,'DEPLOY_PHASE_8K'],[verifyWeb,'VERIFY_WEB_PHASE_8K']]) assert(workflow.includes(token),`Workflow omits ${token}`)
