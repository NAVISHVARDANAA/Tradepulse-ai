...
assert(manifest.phase==='8J'&&manifest.status==='global_dependency_transmission_candidate','Manifest is not Phase 8G')
...
for(const [workflow,token] of [[deployData,'DEPLOY_DATA_PHASE_8J'],[verifyData,'VERIFY_DATA_PHASE_8J'],[buildWeb,'BUILD_PHASE_8J'],[deployWeb,'DEPLOY_PHASE_8J'],[verifyWeb,'VERIFY_WEB_PHASE_8J']]) assert(workflow.includes(token),`Workflow omits ${token}`)
