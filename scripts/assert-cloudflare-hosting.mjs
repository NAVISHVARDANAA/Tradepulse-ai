...
for (const contract of ['VERIFY_WEB_PHASE_5H', 'environment: production', 'npm run test:e2e:production']) {
  assert(verifyWorkflow.includes(contract), `Cloudflare verification contract missing: ${contract}`)
}
assert(manifest.phase === '5H', 'Hosting candidate is not on Phase 5H')
