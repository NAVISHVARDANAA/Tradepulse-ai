...
assert(manifest.phase === '8C', 'Release manifest is not on the current Phase 8C candidate')
...
for (const [name, workflow, confirmation] of [
  ['build', build, 'BUILD_PHASE_8C'],
  ['web deploy', deployWeb, 'DEPLOY_PHASE_8C'],
  ['web verify', verifyWeb, 'VERIFY_WEB_PHASE_8C'],
]) {
...
assert(deployData.includes('DEPLOY_DATA_PHASE_8C'), 'Current data deployment confirmation is missing')
assert(deployData.includes('approved_tester_pilot_smoke.sql'), 'Data deployment omits pilot production smoke')
assert(verifyData.includes('VERIFY_DATA_PHASE_8C'), 'Current data verification confirmation is missing')
