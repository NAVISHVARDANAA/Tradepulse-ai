...
assert(manifest.phase === '8L', 'Release manifest is not on the current Phase 8D candidate')
...
for (const [name, workflow, confirmation] of [
  ['build', build, 'BUILD_PHASE_8L'],
  ['web deploy', deployWeb, 'DEPLOY_PHASE_8L'],
  ['web verify', verifyWeb, 'VERIFY_WEB_PHASE_8L'],
]) {
...
assert(deployData.includes('DEPLOY_DATA_PHASE_8L'), 'Current data deployment confirmation is missing')
assert(deployData.includes('approved_tester_pilot_smoke.sql'), 'Data deployment omits pilot production smoke')
assert(verifyData.includes('VERIFY_DATA_PHASE_8L'), 'Current data verification confirmation is missing')
