...
for (const [name, workflow, confirmation] of [
  ...
  ['web deploy', deployWeb, 'DEPLOY_PHASE_7C'],
  ['web verify', verifyWeb, 'VERIFY_WEB_PHASE_7C'],
]) {
...
assert(deployData.includes('DEPLOY_DATA_PHASE_7C'), 'Current data deployment confirmation is missing')
assert(deployData.includes('approved_tester_pilot_smoke.sql'), 'Data deployment omits pilot production smoke')
assert(verifyData.includes('VERIFY_DATA_PHASE_7C'), 'Current data verification confirmation is missing')
