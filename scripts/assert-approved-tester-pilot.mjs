...
for (const [name, workflow, confirmation] of [
  ['build', build, 'BUILD_PHASE_8M'],
  ['web deploy', deployWeb, 'DEPLOY_PHASE_8M'],
  ['web verify', verifyWeb, 'VERIFY_WEB_PHASE_8M'],
]) {
...
assert(deployData.includes('DEPLOY_DATA_PHASE_8M'), 'Current data deployment confirmation is missing')
assert(deployData.includes('approved_tester_pilot_smoke.sql'), 'Data deployment omits pilot production smoke')
assert(verifyData.includes('VERIFY_DATA_PHASE_8M'), 'Current data verification confirmation is missing')
