...
assert(manifest.phase === '7D', 'Release manifest is not on the current Phase 7D candidate')
...
for (const [name, workflow, confirmation] of [
  ['build', build, 'BUILD_PHASE_7D'],
  ['web deploy', deployWeb, 'DEPLOY_PHASE_7D'],
  ['web verify', verifyWeb, 'VERIFY_WEB_PHASE_7D'],
]) {
...
assert(deployData.includes('DEPLOY_DATA_PHASE_7D'), 'Current data deployment confirmation is missing')
assert(deployData.includes('approved_tester_pilot_smoke.sql'), 'Data deployment omits pilot production smoke')
assert(verifyData.includes('VERIFY_DATA_PHASE_7D'), 'Current data verification confirmation is missing')
