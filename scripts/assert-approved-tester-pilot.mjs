...
assert(manifest.phase === '8B', 'Release manifest is not on the current Phase 8B candidate')
...
for (const [name, workflow, confirmation] of [
  ...
  ['web deploy', deployWeb, 'DEPLOY_PHASE_8B'],
  ['web verify', verifyWeb, 'VERIFY_WEB_PHASE_8B'],
]) {
...
assert(deployData.includes('DEPLOY_DATA_PHASE_8B'), 'Current data deployment confirmation is missing')
assert(deployData.includes('approved_tester_pilot_smoke.sql'), 'Data deployment omits pilot production smoke')
assert(verifyData.includes('VERIFY_DATA_PHASE_8B'), 'Current data verification confirmation is missing')
