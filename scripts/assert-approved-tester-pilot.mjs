...
assert(manifest.phase === '6B', 'Release manifest is not on the current Phase 6B candidate')
...
for (const [name, workflow, confirmation] of [
  ['build', build, 'BUILD_PHASE_6B'],
  ['web deploy', deployWeb, 'DEPLOY_PHASE_6B'],
  ['web verify', verifyWeb, 'VERIFY_WEB_PHASE_6B'],
]) {
...
assert(deployData.includes('DEPLOY_DATA_PHASE_6B'), 'Current data deployment confirmation is missing')
assert(deployData.includes('approved_tester_pilot_smoke.sql'), 'Data deployment omits pilot production smoke')
assert(verifyData.includes('VERIFY_DATA_PHASE_6B'), 'Current data verification confirmation is missing')
