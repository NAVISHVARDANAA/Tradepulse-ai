...
assert(manifest.phase === '8H', 'Release manifest is not on the current Phase 8D candidate')
...
for (const [name, workflow, confirmation] of [
  ['build', build, 'BUILD_PHASE_8H'],
  ['web deploy', deployWeb, 'DEPLOY_PHASE_8H'],
  ['web verify', verifyWeb, 'VERIFY_WEB_PHASE_8H'],
]) {
...
assert(deployData.includes('DEPLOY_DATA_PHASE_8H'), 'Current data deployment confirmation is missing')
assert(deployData.includes('approved_tester_pilot_smoke.sql'), 'Data deployment omits pilot production smoke')
assert(verifyData.includes('VERIFY_DATA_PHASE_8H'), 'Current data verification confirmation is missing')
