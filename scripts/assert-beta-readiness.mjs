...
assert(manifest.phase === '8A', 'Beta manifest is not on Phase 8A')
assert(
  manifest.status === 'global_venue_instrument_intelligence_candidate',
...
for (const contract of ['DEPLOY_PHASE_8A', 'cloudflare/wrangler-action@v3', 'verify:web-deployment', 'test:e2e:production']) {
  assert(deployWorkflow.includes(contract), `Beta deploy workflow contract missing: ${contract}`)
}
for (const contract of ['VERIFY_WEB_PHASE_8A', 'verify:web-deployment', 'test:e2e:production']) {
