...
assert(manifest.phase === '8K', 'Beta manifest is not on Phase 8K')
assert(
  manifest.status === 'global_observation_provenance_quarantine_candidate',
...
assert(
  Array.isArray(manifest.manualPrerequisites) && manifest.manualPrerequisites.length === 16,
...
for (const contract of ['DEPLOY_PHASE_8K', 'cloudflare/wrangler-action@v3', 'verify:web-deployment', 'test:e2e:production']) {
  assert(deployWorkflow.includes(contract), `Beta deploy workflow contract missing: ${contract}`)
}
for (const contract of ['VERIFY_WEB_PHASE_8K', 'verify:web-deployment', 'test:e2e:production']) {
...
console.log(
  'Controlled-beta readiness passed: Cloudflare RC2 candidate, 7 execution locks, 16 manual prerequisites.',
)
