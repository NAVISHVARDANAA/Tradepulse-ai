...
assert(manifest.phase === '8F', 'Beta manifest is not on Phase 8F')
assert(
  manifest.status === 'global_event_intelligence_candidate',
...
assert(
  Array.isArray(manifest.manualPrerequisites) && manifest.manualPrerequisites.length === 13,
...
for (const contract of ['DEPLOY_PHASE_8F', 'cloudflare/wrangler-action@v3', 'verify:web-deployment', 'test:e2e:production']) {
  assert(deployWorkflow.includes(contract), `Beta deploy workflow contract missing: ${contract}`)
}
for (const contract of ['VERIFY_WEB_PHASE_8F', 'verify:web-deployment', 'test:e2e:production']) {
...
console.log(
  'Controlled-beta readiness passed: Cloudflare RC2 candidate, 7 execution locks, 13 manual prerequisites.',
)
