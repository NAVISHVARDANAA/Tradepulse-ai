...
for (const [workflow, contract] of [
  [deployData, 'DEPLOY_DATA_PHASE_8K'],
  [verifyData, 'VERIFY_DATA_PHASE_8K'],
  [buildWeb, 'BUILD_PHASE_8K'],
  [deployWeb, 'DEPLOY_PHASE_8K'],
...
assert(verifyData.includes('migration 055'), 'Current data verification does not prove parity through migration 055')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) {
  assert(guideText.includes('PHASE_8K'), 'A current release guide omits Phase 8D confirmations')
}
