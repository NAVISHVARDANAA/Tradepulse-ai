...
for (const [workflow, contract] of [
  [deployData, 'DEPLOY_DATA_PHASE_8J'],
  [verifyData, 'VERIFY_DATA_PHASE_8J'],
  [buildWeb, 'BUILD_PHASE_8J'],
  [deployWeb, 'DEPLOY_PHASE_8J'],
...
assert(verifyData.includes('migration 054'), 'Current data verification does not prove parity through migration 054')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) {
  assert(guideText.includes('PHASE_8J'), 'A current release guide omits Phase 8D confirmations')
}
