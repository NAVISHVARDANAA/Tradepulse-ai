...
assert(manifest.status === 'options_education_paper_trading_candidate', 'Release status is not the Phase 8A candidate')
...
assert(verifyData.includes('migration 046'), 'Current data verification does not prove parity through migration 046')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) {
  assert(guideText.includes('PHASE_8C'), 'A current release guide omits Phase 8A confirmations')
}
