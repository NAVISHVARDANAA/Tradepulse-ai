...
assert(manifest.status === 'global_provider_certification_isolated_intake_candidate', 'Release status is not the Phase 8A candidate')
...
for (const [workflow, contract] of [
  [deployData, 'DEPLOY_DATA_PHASE_8L'],
  [verifyData, 'VERIFY_DATA_PHASE_8L'],
  [buildWeb, 'BUILD_PHASE_8L'],
  [deployWeb, 'DEPLOY_PHASE_8L'],
...
assert(verifyData.includes('migration 056'), 'Current data verification does not prove parity through migration 056')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) {
  assert(guideText.includes('PHASE_8L'), 'A current release guide omits Phase 8D confirmations')
}
