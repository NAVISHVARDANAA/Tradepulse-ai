...
assert(manifest.status === 'global_country_coverage_candidate', 'Release status is not the Phase 8A candidate')
...
for (const [workflow, contract] of [
  [deployData, 'DEPLOY_DATA_PHASE_8I'],
  [verifyData, 'VERIFY_DATA_PHASE_8I'],
  [buildWeb, 'BUILD_PHASE_8I'],
  [deployWeb, 'DEPLOY_PHASE_8I'],
...
assert(verifyData.includes('migration 053'), 'Current data verification does not prove parity through migration 053')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) {
  assert(guideText.includes('PHASE_8I'), 'A current release guide omits Phase 8D confirmations')
}
