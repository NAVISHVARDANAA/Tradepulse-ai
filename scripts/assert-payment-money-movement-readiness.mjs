...
assert(manifest.status === 'global_evidence_corroboration_candidate', 'Release status is not the Phase 8A candidate')
...
for (const [workflow, contract] of [
  [deployData, 'DEPLOY_DATA_PHASE_8H'],
  [verifyData, 'VERIFY_DATA_PHASE_8H'],
  [buildWeb, 'BUILD_PHASE_8H'],
  [deployWeb, 'DEPLOY_PHASE_8H'],
...
assert(verifyData.includes('migration 052'), 'Current data verification does not prove parity through migration 052')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) {
  assert(guideText.includes('PHASE_8H'), 'A current release guide omits Phase 8D confirmations')
}
