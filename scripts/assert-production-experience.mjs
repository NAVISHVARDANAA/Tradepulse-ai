...
for (const contract of [
  ...
  'Drill through',
  'No transfer, webhook, ledger posting, dispute or refund can be created from this workspace',
...
for (const contract of ['VERIFY_WEB_PHASE_8D', 'npm run test:e2e:production', 'environment: production']) {
  assert(verifyWorkflow.includes(contract), `Manual production verification workflow missing: ${contract}`)
}
for (const contract of ['DEPLOY_PHASE_8D', 'npm run test:e2e:production', 'steps.cloudflare.outputs.deployment-url']) {
...
console.log('Production experience contract passed: 22 routes, runtime failures, Analytics interactions and execution locks guarded.')
