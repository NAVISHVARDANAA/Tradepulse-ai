...
assert(manifest.status === 'global_event_intelligence_candidate', 'Release status is not the Phase 8A candidate')
...
for (const [workflow, confirmation] of [
  ...
  [buildWeb, 'BUILD_PHASE_8F'], [deployWeb, 'DEPLOY_PHASE_8F'],
...
assert(verifyData.includes('migration 050'), 'Data verification omits the current migration parity check')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) assert(guideText.includes('PHASE_8F'), 'A current release guide omits Phase 8D')
