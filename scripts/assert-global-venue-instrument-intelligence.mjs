...
assert(manifest.status === 'global_dependency_transmission_candidate', 'Release status is not the Phase 8A candidate')
...
for (const [workflow, confirmation] of [
  ...
  [buildWeb, 'BUILD_PHASE_8J'], [deployWeb, 'DEPLOY_PHASE_8J'],
  [verifyWeb, 'VERIFY_WEB_PHASE_8J'],
...
assert(verifyData.includes('migration 054'), 'Data verification omits the current migration parity check')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) assert(guideText.includes('PHASE_8J'), 'A current release guide omits Phase 8D')
