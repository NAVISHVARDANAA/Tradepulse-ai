...
assert(manifest.status === 'controlled_live_rollout_candidate', 'Release status is not the Phase 8A candidate')
...
for (const [workflow, confirmation] of [
  ...
  [buildWeb, 'BUILD_PHASE_8G'], [deployWeb, 'DEPLOY_PHASE_8G'],
...
assert(verifyData.includes('migration 051'), 'Data verification omits the current migration parity check')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) assert(guideText.includes('PHASE_8G'), 'A current release guide omits Phase 8D')
