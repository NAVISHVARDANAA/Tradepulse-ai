...
assert(manifest.status === 'global_observation_provenance_quarantine_candidate', 'Release status is not the Phase 8A candidate')
...
for (const [workflow, confirmation] of [
  ...
  [buildWeb, 'BUILD_PHASE_8K'], [deployWeb, 'DEPLOY_PHASE_8K'],
  [verifyWeb, 'VERIFY_WEB_PHASE_8K'],
...
assert(verifyData.includes('migration 055'), 'Data verification omits the current migration parity check')
...
for (const guideText of [releaseGuide, hostingGuide, supabaseGuide]) assert(guideText.includes('PHASE_8K'), 'A current release guide omits Phase 8D')
